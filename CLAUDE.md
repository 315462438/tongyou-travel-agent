...
**其实是一次静默的硬失败，被完整地误读成了设计问题。**
① **视觉端单边上限 8192px**。那张图 1800×25242，每次都 400，而报文写的是
「unsupported image，请确认格式是 webp/png/jpeg/gif」——**格式是对的（png 就在白名单里），
越界的是尺寸**。照着报文查格式会一路走错。逐档实测一次排掉三个假设：不是字节数
（941KB 过 / 988KB 挂，而 1046KB、1207KB 同样挂，三档交错）、不是宽高比
（1024×14360 压了宽度照样只挂在高度上）、不是格式。**当报文给的原因与手上的事实矛盾时
相信事实**，坑见 `docs/pitfalls/报错说格式不支持其实是尺寸越界.md`。
修法是**切片不缩小**（25242→8192 是 0.32 倍，行程单上的字直接糊掉）：新增
`app/agent/image_prep.py` 按高度切片、相邻片重叠 80px，多片放进**同一次**调用
（分多次要付 N 份 prompt，还会割裂同一份文档——Day3 的「续住」在片 2、酒店名在片 1）。
⚠️ **片数不够时先整体缩小再切，绝不切前 N 片丢尾部**：后者让模型拿着半份行程自信作答，
8 天说成 5 天且不报错。变异检验：改成截断该用例立刻红。新增依赖 **Pillow**（切片必须解码，
stdlib 没有编解码器；1800×25242 解码约 180MB，服务器 available 4.3G + 10G swap 够）。
顺带挖出**第二个我们自己加的静默截断**：渲染时 `uniq[:15]` 把 8 天行程砍在 Day 2 ——
上限是渲染侧的、不是模型产出上限，而用户问的恰恰是「路线合不合适」。
② **追问熔断从来没生效过**。`_recent_clarify_rounds` 靠「≤60 字且问号结尾」认自己发过的
反问，而模型在问句后顺手接了一句「我好帮您推荐周边美食和检查路线。」→ 句尾变「。」、
长度也过了 60 → 计数恒为 0，**`clarify_max_rounds` 永远够不着，问 10 次也是第 0 轮**。
**这条消息是我们自己发的，发的时候百分之百知道它是澄清，却回过头去猜自己的文案**——
中间隔着一个自由发挥的模型。现在写 `meta.clarify`、读 `meta.clarify`，文案启发式降级为
只认改造前的老消息；`_is_clarify_continuation` 与 `memory._first_guide_reply` 一并改用同一
判据。**失效方式最坏的一点是它不报错**：熔断代码一直在、单测一直绿（单测喂的是
「想去哪里呢？」这种教科书式短问句，正好落在启发式命中的那一侧），只有真实措辞会让它
失效且线上没有任何日志。回归用例**直接用线上那句原文**做输入。坑见
`docs/pitfalls/别靠文案认出自己发过的消息.md`（一般化：反问→`meta.clarify` 不是「问号结尾」，
占位→`meta.streaming` 不是「内容为空」，**凡系统读回自己写过的东西，判据必须是当时记下的
标记而非产物的形状**；同类：Phase 14「看最后一条」、Phase 108 `updated_at` 兼表两种语义）。
③ **反问要对得上原因**。`_image_unreadable_this_turn`（读 `_image_unreadable` 落的 progress
标记，轮次边界与 `clear_plain_progress` 同一套）命中时走 `_BLIND_ASKS` 三级升级文案，
每轮换一种说法且给出新的出路（打城市名 / 拆成单页重发 / 说「你定」）。
④ **刻意的例外：读不出图时不代选**（`forced` 加 `and not blind`）。Phase 68 的强制代选是为
「用户没想好去哪」设计的；这里我们有**正面证据**说明用户手上有确定目的地，只是没读出来，
替他挑一个是拿几分钟生成换一份必然不相干的行程。代价不对称：再问一句是一次往返，
猜错是整轮作废且用户更确信「它没看我的图」。**但 `let_agent_decide` 仍照常代选**——
那是用户自己的授权，与我们读没读出图无关。
⑤ **失败必须可见**：此前每处失败都是 `logger.warning` + 返回空串，而前面刚播过
「正在看你发的 N 张图…」——**进度说读了、实际没读**。凡「读取外部内容失败后继续跑」的
地方都要检查：前面有没有播过一条声称做了这件事的进度？有的话失败就必须同样可见。
长截图另播一条「分 N 段读，稍慢一点」（实测 16–43s，Phase 71：静默空隙才是流失主因）。
线上复验（用真的挂过的那张图走真实代码路径）：4 片 / 一次调用 / 目的地读出吉隆坡·仙本那·
亚庇 / 10-01–10-08 八天齐全 / Day1–Day8 全部。计划
`docs/task_plans/长截图视觉读取修复-2026-08-27.md`，用例
`docs/test_cases/长截图视觉读取修复-验收用例.md`（`test_image_prep.py` 13 +
`test_vision.py` 新增 7 + `test_image_failure_visible.py` 3 + `test_clarify_escalation.py` 14，
全量 1444 passed）。

**Phase 110 — 首页去模板化**：空状态首屏此前按输入形状预先分流成四类模板
（收藏整理 / 预算推荐 / 旅行预演 / 其他），把用户的话套进 `buildJourneyPreviewPrompt`
这类模板串再发给后端。真实反馈：第一轮上传图片说「分析一下」，请求仍被套成模板问题。
**智能体该更通用，模板化的前端分流是在替后端做它自己会做的判断。** 现在只留两类——
含公开链接走收藏整理（那条路要提 URL），其余**原样把用户的话发出去**
（`const prompt = idea.trim()`）；删掉 `isCompactDestinationIdea` /
`buildJourneyPreviewPrompt` / `buildBudgetRoulettePrompt` 及其输入类型。
⚠️ 连带的后端缺口留作后续：`resolve_route` 与 `Preference` **只读文本不读图**，
前端不再预先塑形之后这条更要紧了（Phase 105 ⑥ 已列为「等真实使用数据再定」）。

**Phase 110.1 — Ctrl+V 粘贴图片**：四处 paste 处理器都只读 `e.clipboardData.files`，
而**从网页右键复制的图片只出现在 `clipboardData.items` 里**（Safari 亦然）→ 粘贴毫无反应
且不报错。新增纯函数 `extractClipboardImages`（两个通道都读，按 `name|size|lastModified`
去重——同一张图可能两边各出现一次，不去重会上传两遍），四处统一替换。另加 document 级
兜底监听：焦点不在可编辑元素上时才接管，否则会和 textarea 自己的 handler 重复处理同一次
粘贴。护栏改成钉 `extractClipboardImages(e.clipboardData` 并**禁止退回**
`Array.from(e.clipboardData.files)`（原来钉的是 `f.type.startsWith('image/')` 这种实现细节，
一改就红）。

**Phase 109 — 连接管理页**：携程/小红书的登录态从「攻略生成中途弹扫码」剥离成独立入口
（`app/agent/connectors.py` + `app/api/connectors_api.py` + `connect_session.py`），
用户可主动连接/断开，断开 = `pool.restart(user_id)` 杀进程后删 profile 目录再清
`TravelSiteLogin`（**先杀后删**，进程占着目录删不掉）。界面按豆包的两级渐进披露
（列表 → 详情），不是平铺一张大表。三个上线后才暴露的问题：函数体内的**惰性导入**在跑到
之前是隐形的（`app.tools.chrome_mcp` 实际叫 `mcp_client`，测试因为 mock 掉了 `_run` 从未执行
到那行）→ 补 AST + `importlib` 护栏；截图 URL 前缀（后端只回 token，前端拼 `API`，
并断言 `view()` 的返回值不许以 `/` 开头）；携程默认展示的是密码表单不是二维码
（进等待前先 `find_and_click("扫码登录")`）。坑见
`docs/pitfalls/函数体内的惰性导入在跑到之前是隐形的.md`。
